...
- The pool of allocatable IPs comes from `pool.json` (the addresses the
  ...
  gateway and the broadcast address (so a `/24` with the `init` default
  pool-size of 140 yields 140 addresses while the `adopt` default of 253
  covers every usable host address); Volcengine
